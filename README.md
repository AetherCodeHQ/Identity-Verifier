...
> Identity-Verifier - Open source tool by AetherCodeHQ

`cli` `golang` `identity` `security` `verification`

## What is it?

**Identity Verifier** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Identity-Verifier.git
...
```bash
# Basic usage
./Identity-Verifier --path ./target

# With options
./Identity-Verifier --path ./target --format json --output report.json

# Verbose mode
./Identity-Verifier --path ./target --verbose
```
...
### Basic scan
```bash
./Identity-Verifier --path ./my-project
```

### JSON report
```bash
./Identity-Verifier --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Identity Verifier
  run: ./Identity-Verifier --path . --format json --output report.json
```
